...
for pathFrag in results local rete_incremental_$suffix viatra
do
	generalResultPath+=/
...
do	
	...
	do
		...
		do
			...
			timeout 3h java -jar -Xms350g -Xmx350g incremental/execute_ldbc_query_viatra_incremental.jar $dataDir $cleanRule $logMem > $resultFile
			retval=$?
			if [ "$retval" == 124 ]
			then
				echo -e '\t\t\tTIMEOUT'
