...
for pathFrag in results local rete_incremental_$suffix periodic_$period
do
	generalResultPath+=/
...
do	
	...
	do
		...
		do
			...
			timeout 3h java -jar -Xms350g -Xmx350g incremental/execute_ldbc_query_periodic_rete_incremental.jar $dataDir $rulesDir'/interactive_'$ruleNumber'.mlsp' $period $logMem > $resultFile
			retval=$?
			if [ "$retval" == 124 ]
			then
				echo -e '\t\t\tTIMEOUT'
