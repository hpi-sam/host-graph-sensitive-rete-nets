...
for pathFrag in results local rete_incremental_$suffix brute
do
	generalResultPath+=/
...
do	
	...
	do
		...
		do
			...
			timeout 3h java -jar -Xms350g -Xmx350g incremental/execute_ldbc_query_brute_force_rete_incremental.jar $dataDir $rulesDir'/interactive_'$ruleNumber'.mlsp' $logMem > $resultFile
			retval=$?
			if [ "$retval" == 124 ]
			then
				echo -e '\t\t\tTIMEOUT'
