...
for pathFrag in results local rete_incremental_$suffix dynamic_$lookahead
do
	generalResultPath+=/
...
do	
	...
	do
		...
		do
			...
			timeout 3h java -jar -Xms350g -Xmx350g incremental/execute_ldbc_query_dynamic_rete_incremental_$lookahead'.jar' $dataDir $rulesDir'/interactive_'$ruleNumber'.mlsp' $logMem > $resultFile
			retval=$?
			if [ "$retval" == 124 ]
			then
				echo -e '\t\t\tTIMEOUT'
