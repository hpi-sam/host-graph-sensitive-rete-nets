...
for pathFrag in results local rete_evolution_$suffix dynamic
do
	generalResultPath+=/
...
do	
	...
	do
		...
		do
			...
			timeout 10h java -jar -Xms350g -Xmx350g evolution/execute_ldbc_query_dynamic_rete_evolution.jar $dataDir $rulesDir'/interactive_'$ruleNumber'.mlsp' $logMem > $resultFile
			retval=$?
			if [ "$retval" == 124 ]
			then
				echo -e '\t\t\tTIMEOUT'
