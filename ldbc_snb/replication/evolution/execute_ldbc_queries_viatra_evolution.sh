...
for pathFrag in results local rete_evolution_$suffix viatra
do
	generalResultPath+=/
...
do	
	...
	do
		...
		do
			...
			timeout 10h java -jar -Xms350g -Xmx350g evolution/execute_ldbc_query_viatra_evolution.jar $dataDir $cleanRule $logMem > $resultFile
			retval=$?
			if [ "$retval" == 124 ]
			then
				echo -e '\t\t\tTIMEOUT'
